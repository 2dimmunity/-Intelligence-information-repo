...
def depth2ptc_with_mask(depth, calib, mask):
    vu = np.indices(depth.shape).reshape(2, -1).T
    vu[:, 0], vu[:, 1] = vu[:, 1], vu[:, 0].copy()
    uv = vu
    uv_depth = np.column_stack((uv.astype(float), depth.reshape(-1)))
    uv_depth = np.column_stack((uv_depth, mask.reshape(-1)))
    return calib.project_image_to_rect_with_mask(uv_depth)
...
def convert_and_save_with_mask(args, i):
    depth_map = np.load(osp.join(
                args.input_path, "{:06d}.npy".format(i)))
    calib = Calibration(
        osp.join(args.calib_path, "{:06d}.txt".format(i)))
    mask = np.load(osp.join(
                args.mask_path, "{:06d}.npy".format(i)))
    ptc = filter_height(
        calib.project_rect_to_velo_with_mask(depth2ptc_with_mask(depth_map, calib, mask)))
...
if __name__ == "__main__":
    ...
    if args.i is not None:
        ...
        res = []
        for i in idx_list:
            res.append((i, pool.apply_async(convert_and_save_with_mask, args=(args, i),
                                            callback=update)))
